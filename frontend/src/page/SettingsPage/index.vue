    ...
    <div class="h-screen container mx-auto px-4 pt-20 max-w-5xl">
        <div class="space-y-6">
            <div class="flex flex-col gap-1">
                ...
                <p class="text-sm text-base-content/7o">Choose a theme for your chat interface</p>
            </div>
            <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
                <button v-for="t in THEME" :key="t" class=" group flex flex-col items-center gap-1.5 p-2 rounded-lg
                        transition-colors" :class="theme === t ? 'bg-base-200' : 'hover:bg-base-200/50'"
                    @click="setTheme(t)">
                    ...
                    <div class="max-w-lg mx-auto">
                        ...
                        <div class="bg-base-100 rounded-xl shadow-sm overflow-hidden">
                            ...
                            <div class="p-4 space-y-4 min-h-[200px] max-h-[200px] overflow-y-auto bg-base-100">
                                <div v-for="message in PREVIEW_MESSAGES" :key=message.id class="flex"
                                    :class="message.isSent ? 'justify-end' : 'justify-star'">
                                    <div class=" max-w-[80%] rounded-xl p-3 shadow-sm"
                                        :class="message.isSent ? 'bg-primary text-primary-content' : 'bg-base-200'">
                                        <p class="text-sm">{{ message.content }}</p>
                                        <p class="text-[10px] mt-1.5"
                                            :class="message.isSent ? 'text-primary-content/70' : 'text-base-content/70'">
                                            12:00 PM
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div class="p-4 border-t border-base-300 bg-base-100">
                                <div class="flex gap-2">
                                    <input type="text" class="input input-bordered flex-1 text-sm h-10"
